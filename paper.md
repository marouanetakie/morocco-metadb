...
The current release contains 32 plant species (all from Morocco), 186 phytochemical compounds in five classes (terpenoids 89, flavonoids 47, polyphenols 28, alkaloids 15, fatty acids 2), 122 bioactivity measurements, and 283 references with verified PubMed identifiers. Three entries are chemically notable: *Peganum harmala* contributes the first alkaloid-class profile in the database (harmine, harmaline, vasicine); *Retama monosperma* adds isoflavones (genistein, biochanin A, formononetin); *Withania somnifera* brings withanolide steroidal lactones with the strongest cytotoxicity in the database (MTT IC50 = 15.8 µg/mL).
